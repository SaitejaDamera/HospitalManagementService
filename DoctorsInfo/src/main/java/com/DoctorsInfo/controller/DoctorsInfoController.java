...
public class DoctorsInfoController {
	...
	@DeleteMapping("/deletedoctors/{doctorsId}")
	public ResponseEntity deleteDoctor(@PathVariable(value="doctorsId") Long id) {
		 doctorsInfoService.deleteDoctorsById(id);
		 return new ResponseEntity(HttpStatus.OK);
	}
	
	@PutMapping("/updatedoctor")
	public ResponseEntity updateDoctorsInfo(@RequestBody Doctors doctors) {
		doctorsInfoService.updateDoctorInfo(doctors);
		return new ResponseEntity(HttpStatus.OK);
	}
